...
def upsert_country(cursor, country_code: str) -> int:
    ...
    cursor.execute(
        ...
        RETURNING id
        """,
        (country_code, country_code),  # country_name = code en attendant un référentiel
    )
    row = cursor.fetchone()
    if row:
        return row[0]
...
def upsert_indicator(cursor, indicator_code: str) -> int:
    ...
    cursor.execute(
        ...
        RETURNING id
        """,
        (indicator_code, indicator_code),  # indicator_name = code, à enrichir plus tard
    )
    row = cursor.fetchone()
    if row:
        return row[0]
...
def transform_and_load() -> int:
    ...
    Ignore les doublons via ON CONFLICT sur (country_id, indicator_id, year).
    ...
    try:
        ...
        for (record,) in rows:
            ...
            # ON CONFLICT avec colonnes explicites (compatible avec UNIQUE INDEX ou CONSTRAINT)
            cursor.execute(
                """
        ...
        conn.commit()
        logger.info(
            f"[TRANSFORM] {total_inserted} lignes insérées en analytics.health_data"
        )
